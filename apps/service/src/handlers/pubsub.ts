 ...
 */
async function handleBackgroundOperations(params: BackgroundOperationParams): Promise<void> {
	const { env, projectId, distributedId, distributedIdWithLocationHint, locationHint, logTag } = params;
	try {
		// Register the shard and distributedId in the redis cache
		console.log(`${logTag} [Background] Registering channel and shard in Redis...`);
		const registrationResult = await registerChannelAndShard(env, projectId, distributedIdWithLocationHint, locationHint);
		console.log(`${logTag} [Background] registerChannelAndShard result:`, registrationResult);

		if (!registrationResult) {
			console.error(`${logTag} [Background] Failed to register channel and shard in Redis`);
			return;
		}
		...
		console.log(`${logTag} [Background] Fetching available shards for distributedId:`, distributedId);
		...
		const shardUpdatePromises = availableShards.map(async (shard) => {
			try {
				...
				console.log(`${logTag} [Background] Setting shards in localStorage for shard:`, shard);
				await shardStub.setShardsInLocalStorage(availableShards);
				console.log(`${logTag} [Background] Successfully updated shard:`, shard);
			} catch (error) {
				console.error(`${logTag} [Background] Failed to update shard ${shard}:`, error);
		...
		console.log(`${logTag} [Background] All background operations completed successfully`);
	} catch (error) {
		console.error(`${logTag} [Background] Error in background operations:`, error);
...
export async function pubsub(props: HandlerProps): Promise<Response> {
	...
	console.log(`${logTag} Received request with locationHint: ${props.locationHint}`);

	// Extract grant token from query or header
	const grantResult = extractGrantToken(props.request);
	if (!grantResult) {
		console.warn(`${logTag} No grant token provided in query parameter or X-Erebus-Grant header`);
		return createErrorResponse('Unauthorized: Grant token required in query parameter or X-Erebus-Grant header', 401);
	}

	const { grant, source } = grantResult;
	console.log(`${logTag} Grant token extracted from ${source}: ${grant.substring(0, 100)}...`);

	// Verify the grant token signature and extract claims
	console.log(`${logTag} Verifying grant token...`);
	const verifiedGrant = await verifyRequestToken(grant, props.env.PUBLIC_KEY_JWK);
	if (!verifiedGrant) {
		console.warn(`${logTag} Invalid grant token - signature verification failed`);
		return createErrorResponse('Unauthorized: Invalid grant token signature', 401);
	}
	...
	const distributedIdWithLocationHint = DistributedKey.appendLocationHint(distributedId, props.locationHint);
	console.log(`${logTag} Computed location-aware distributedId: ${distributedIdWithLocationHint}`);
	...
	console.log(`${logTag} Created Durable Object stub with locationHint:`, props.locationHint);
	 ...
	 */
	console.log(`${logTag} Forwarding request to Durable Object with location hint: ${props.locationHint}`);
	...
	try {
		...
		console.log(`${logTag} Response received from Durable Object (status: ${response.status})`);
		return response;
	} catch (error) {
		console.error(`${logTag} Error forwarding request to Durable Object:`, error);
		return createErrorResponse('Internal Server Error: Failed to establish connection', 500);
	}
}
