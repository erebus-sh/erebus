 ...
 */
export function getLocationHint(location: Location = {}): DurableObjectLocationHint {
	if (!location.continentCode && !location.point) return 'enam';
	const areas = continentCodes[location.continentCode || 'AN'];
	let closest = areas[0];
	let shortestDistance = Infinity;

	if (areas.length === 1 || !location.point) return closest as DurableObjectLocationHint;
