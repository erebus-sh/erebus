...
export default {
	...
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (method === 'POST' && path === '/v1/root/command') {
			...
			if (!rootApiKey) {
				return new Response(JSON.stringify({ error: 'Unauthorized you must provide a root api key' }), {
					status: 401,
			...
			if (rootApiKey !== env.ROOT_API_KEY) {
				return new Response(JSON.stringify({ error: 'Unauthorized you must provide a valid root api key' }), {
					status: 401,
					headers: { 'Content-Type': 'application/json' },
				});
			}
			...
			if (!command.success) {
				return new Response(JSON.stringify({ error: 'Unauthorized you must provide a valid command' }), {
					status: 401,
					headers: { 'Content-Type': 'application/json' },
				});
			}
		...
		if (upgrade !== 'websocket') {
			return new Response(JSON.stringify({ error: 'Erebus service expects a WebSocket request' }), {
				status: 400,
				headers: { 'Content-Type': 'application/json' },
			});
		}
