...
export class ShardManager extends BaseService {
	...
	async getAvailableShards(): Promise<string[]> {
		...
		const shards = (await this.getStorageValue<string[]>(ShardManager.AVAILABLE_SHARDS_KEY, [])) || [];
	...
	async setShardsInLocalStorage(shards: string[]): Promise<void> {
		this.logDebug(`[SET_SHARDS] Setting shards in local storage - count: ${shards.length}`);

		// Get current location to filter out self-references
		const myLocation = await this.getStorageValue<string>(ShardManager.LOCATION_HINT_KEY);
		this.logDebug(`[SET_SHARDS] My location hint: ${myLocation}`);

		// Deduplicate and filter out self-location
		const uniqueShards = Array.from(new Set(shards)).filter((shard) => shard !== myLocation);

		this.logDebug(`[SET_SHARDS] Unique shards after filtering: ${uniqueShards.length}`);

		// Check if update is needed (optimization to avoid unnecessary writes)
		const existingShards = (await this.getStorageValue<string[]>(ShardManager.AVAILABLE_SHARDS_KEY, [])) || [];
		...
		this.logDebug(`[SET_SHARDS] Shards stored successfully (count: ${uniqueShards.length})`);
	}
	...
	async getLocationHint(): Promise<string | null> {
		const location = await this.getStorageValue<string>(ShardManager.LOCATION_HINT_KEY);
	...
	async getRemoteShards(): Promise<string[]> {
		...
		const [allShards, myLocation] = await Promise.all([this.getAvailableShards(), this.getLocationHint()]);
	...
	async shouldBroadcastToShards(): Promise<boolean> {
		...
		this.logDebug(`[SHOULD_BROADCAST] Should broadcast to shards: ${shouldBroadcast}`);
