...
import { SocketSendResult, MessageMetrics, BroadcastConfig, DEFAULT_BROADCAST_CONFIG, PublishMessageParams, ServiceContext } from './types';
...
export class MessageBroadcaster extends BaseService {
	...
	async publishMessage(params: PublishMessageParams): Promise<void> {
		const { messageBody, senderClientId, subscriberClientIds, projectId, channelName, topic, seq, tIngress } = params;
		...
		this.logDebug(`[PUBLISH_MESSAGE] Total WebSocket connections: ${sockets.length}`);
		...
		const serializedMessage = MessageBroadcaster.TEXT_ENCODER.encode(JSON.stringify(messageBody));

		// Process sockets in batches to prevent event loop blocking
		await this.procesSocketBatches(sockets, serializedMessage, senderClientId, subscriberClientIds, topic, sentToClients, metrics);
		...
		await this.runBackgroundTasks(messageBody, projectId, channelName, topic, seq, subscriberClientIds);
	}
	...
	private async procesSocketBatches(
		...
		for (let i = 0; i < sockets.length; i += batchSize) {
			...
			const batchPromises = batch.map((socket) =>
				this.processSingleSocket(socket, serializedMessage, senderClientId, subscriberClientIds, topic, sentToClients),
			);
	...
	private async processSingleSocket(
		...
		try {
			...
			if (!attachment) {
				this.logDebug(`[PROCESS_SOCKET] Socket does not have a grant attachment`);
			...
			if (!grant.success) {
				this.logDebug(`[PROCESS_SOCKET] Socket does not have a valid grant attachment`);
			...
			const topicAccess = grant.data.topics.find((t) => t.topic === topic || t.topic === '*');

			if (topicAccess?.scope === Access.Huh) {
				// Special "Huh" scope - send informational message
				return await this.sendHuhMessage(socket, recipientClientId, sentToClients);
			} else if (topicAccess?.scope === Access.Read || topicAccess?.scope === Access.ReadWrite) {
				// Regular read access - send the actual message
				return await this.sendMessage(socket, serializedMessage, recipientClientId, senderClientId, subscriberClientIds, sentToClients);
			} else {
				this.logDebug(`[PROCESS_SOCKET] Socket ${grant.data.userId} lacks read scope for topic: ${topic}`);
	 ...
	 * @returns Promise resolving to send result
	 */
	private async sendHuhMessage(socket: WebSocket, recipientClientId: string, sentToClients: Set<string>): Promise<SocketSendResult> {
	 ...
	 */
	private async handleBackpressure(socket: WebSocket): Promise<SocketSendResult | null> {
	 ...
	 */
	private updateMetricsFromResults(metrics: MessageMetrics, results: SocketSendResult[]): void {
	...
	private async runBackgroundTasks(
		...
		await Promise.all([
			...
			this.updateLastSeenBulk(subscriberClientIds, projectId, channelName, topic, seq),
		]).catch((error) => {
	 ...
	 */
	private async bufferMessage(messageBody: MessageBody, projectId: string, channelName: string, topic: string, seq: string): Promise<void> {
		await this.messageBuffer.bufferMessage(messageBody, projectId, channelName, topic, seq);
	}

	private async updateLastSeenBulk(clientIds: string[], projectId: string, channelName: string, topic: string, seq: string): Promise<void> {
