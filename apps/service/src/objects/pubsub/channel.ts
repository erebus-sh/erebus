 ...
 */
export class ChannelV1 extends ErebusPubSubService implements MessageBroadcastCoordinator {
	...
	constructor(ctx: DurableObjectState, env: Env) {
		...
		this.messageBroadcaster = new MessageBroadcaster(this.serviceContext, this.messageBuffer);
		...
		this.log('[CONSTRUCTOR] ChannelV1 instance created with all services initialized');
	}
	...
	async fetch(request: Request): Promise<Response> {
		...
		try {
			...
			this.log(`[FETCH] Error setting up WebSocket connection: ${error}`, 'error');
	...
	async webSocketClose(ws: WebSocket, code?: number, reason?: string, wasClean?: boolean): Promise<void> {
		...
		this.log(`[CLOSE] WebSocket close event - code: ${code}, reason: ${reasonPreview}, wasClean: ${wasClean}`);
	...
	async broadcastToAllShardsAndUpdateState(
		...
		this.logDebug(`[BROADCAST] Starting broadcast - senderId: ${senderClientId}, topic: ${topic}`);
		...
		if (shards.length > 0) {
			...
			for (const shard of shards) {
				...
				remotePublishPromises.push(
					channelStub.publishMessage(filledPayload, senderClientId, subscriberClientIds, projectId, channelName, topic, seq, tIngress),
				);
			}
		}
		...
		this.logDebug(`[BROADCAST] Broadcast completed - duration: ${broadcastDuration.toFixed(2)}ms`);
	}
