...
export class SubscriptionManager extends BaseService {
	...
	async subscribeToTopic(params: SubscriptionParams): Promise<void> {
		...
		await this.transaction(async (txn) => {
			...
			if (current.length >= PUBSUB_CONSTANTS.MAX_SUBSCRIBERS_PER_TOPIC) {
				...
				throw new Error(`Channel is full and has more than ${PUBSUB_CONSTANTS.MAX_SUBSCRIBERS_PER_TOPIC} subscribers`);
			}
			...
			if (!current.includes(clientId)) {
				...
				this.logDebug(`[SUBSCRIBE] Client added to subscribers, new count: ${updated.length}`);
			} else {
	...
	async unsubscribeFromTopic(params: SubscriptionParams): Promise<void> {
		...
		await this.transaction(async (txn) => {
			const current = (await txn.get<string[]>(key)) ?? [];
			this.logDebug(`[UNSUBSCRIBE] Current subscribers count: ${current.length}`);

			// Remove client from subscribers
			const updated = current.filter((id) => id !== clientId);
			await txn.put(key, updated);
			this.logDebug(`[UNSUBSCRIBE] Client removed from subscribers, new count: ${updated.length}`);
		});
	 ...
	 */
	async getSubscribers(projectId: string, channelName: string, topic: string): Promise<string[]> {
		this.logDebug(`[GET_SUBSCRIBERS] Getting subscribers for topic: ${topic}`);

		const key = STORAGE_KEYS.subscribers(projectId, channelName, topic);
		const subscribers = (await this.getStorageValue<string[]>(key, [])) || [];

		this.logDebug(`[GET_SUBSCRIBERS] Found ${subscribers.length} subscribers for key: ${key}`);
		return subscribers;
	}
	 ...
	 */
	async getSubscriberCounts(projectId: string, channelName: string, topics: string[]): Promise<Map<string, number>> {
		this.logDebug(`[GET_SUBSCRIBER_COUNTS] Getting counts for ${topics.length} topics`);

		const counts = new Map<string, number>();

		// Fetch all subscriber lists in parallel
		const subscriberLists = await Promise.all(topics.map((topic) => this.getSubscribers(projectId, channelName, topic)));
		...
		this.logDebug(`[GET_SUBSCRIBER_COUNTS] Retrieved counts for ${counts.size} topics`);
		return counts;
	}
	 ...
	 */
	async bulkUnsubscribe(clientId: string, projectId: string, channelName: string, topics: string[]): Promise<void> {
		this.logDebug(`[BULK_UNSUBSCRIBE] Unsubscribing client ${clientId} from ${topics.length} topics`);

		// Process all unsubscriptions in parallel for performance
		await Promise.all(topics.map((topic) => this.unsubscribeFromTopic({ topic, projectId, channelName, clientId })));
	 ...
	 */
	async getActiveTopics(projectId: string, channelName: string): Promise<string[]> {
		this.logDebug(`[GET_ACTIVE_TOPICS] Getting active topics for channel: ${channelName}`);
	 ...
	 */
	async getTotalSubscriptionCount(projectId: string, channelName: string): Promise<number> {
		this.logDebug(`[GET_TOTAL_SUBS] Getting total subscription count for channel: ${channelName}`);
