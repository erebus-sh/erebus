...
export abstract class ErebusPubSubService extends DurableObject<Env> {
	 ...
	 */
	protected log(message: string, level: 'log' | 'warn' | 'error' = 'log', prefix?: string): void {
	 ...
	 */
	protected async sendWebSocketMessage(ws: WebSocket, data: unknown): Promise<void> {
	 ...
	 */
	protected safeCloseWebSocket(ws: WebSocket, code?: number, reason?: string): void {
		try {
			if (ws.readyState === WebSocket.READY_STATE_OPEN) {
				ws.close(code, reason);
				this.logDebug(`[CLOSE_WS] WebSocket closed (code: ${code}, reason: ${reason})`);
			}
		} catch (error) {
	 ...
	 */
	protected async transaction<T>(operation: (txn: DurableObjectTransaction) => Promise<T>): Promise<T> {
		try {
	 ...
	 */
	protected async getStorageValue<T>(key: string, defaultValue?: T): Promise<T | undefined> {
		try {
	...
	protected async deleteStorageValue(key: string): Promise<boolean> {
		try {
			...
			this.log(`[STORAGE_DELETE] Failed to delete key '${key}': ${error}`, 'error');
			throw error;
		}
	}
	 ...
	 */
	protected async listStorage<T>(options?: DurableObjectListOptions): Promise<Map<string, T>> {
		try {
