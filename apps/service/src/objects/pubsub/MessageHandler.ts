...
export class MessageHandler extends BaseService {
	...
	async handleWebSocketMessage(ws: WebSocket, message: string): Promise<void> {
		...
		try {
			...
			this.logDebug(`[WS_MESSAGE] Message parsed successfully, packetType: ${parsed.packetType}`);

			const envelopeResult = PacketEnvelopeSchema.safeParse(parsed);
			if (!envelopeResult.success) {
				this.logError(`[WS_MESSAGE] Invalid packet envelope: ${envelopeResult.error}`);
				this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid packet format');
				return;
			}
		...
		this.logDebug(`[WS_MESSAGE] Processing packet type: ${envelope.packetType}`);
		...
		try {
			switch (envelope.packetType) {
				...
				case 'publish':
					...
					break;
				default:
					this.logError(`[WS_MESSAGE] Unknown packet type: ${(envelope as any).packetType}`);
					this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Unknown packet type');
			}
		} catch (error) {
			this.logError(`[WS_MESSAGE] Error processing ${envelope.packetType} packet: ${error}`);
			this.closeWebSocketWithError(ws, WsErrors.InternalServerError, 'Processing failed');
		}
	}
	 ...
	 */
	private async handleConnectPacket(ws: WebSocket, envelope: PacketEnvelope): Promise<void> {
		...
		if (envelope.packetType !== 'connect') {
			this.logError(`[WS_CONNECT] Invalid packet type for connect handler: ${envelope.packetType}`);
			this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid packet type');
			return;
		}
		...
		try {
			...
			if (!payload.success) {
				...
				this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid grant format');
				return;
			}
			...
			this.logDebug(`[WS_CONNECT] JWT verified and grant attached for userId: ${attachment.userId}`);
		} catch (error) {
			this.logError(`[WS_CONNECT] JWT verification failed: ${error}`);
			this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Authentication failed');
		}
	}
	 ...
	 * @param envelope - Parsed packet envelope
	 */
	private async handleSubscribePacket(ws: WebSocket, envelope: PacketEnvelope): Promise<void> {
		...
		if (envelope.packetType !== 'subscribe') {
			this.logError(`[WS_SUBSCRIBE] Invalid packet type for subscribe handler: ${envelope.packetType}`);
			this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid packet type');
			return;
		}
		...
		this.logDebug(`[WS_SUBSCRIBE] Subscribe request - clientId: ${clientId}, ` + `topic: ${topic}, channel: ${channelName}`);
		...
		const isAuthorized = grant.data.topics.some((t: Grant['topics'][0]) => t.topic === topic || t.topic === '*');
		if (!isAuthorized) {
		...
		try {
			...
			this.logDebug(`[WS_SUBSCRIBE] Client authorized, proceeding with subscription`);
			...
			this.closeWebSocketWithError(ws, WsErrors.InternalServerError, 'Subscription failed');
		}
	}
	 ...
	 * @param envelope - Parsed packet envelope
	 */
	private async handleUnsubscribePacket(ws: WebSocket, envelope: PacketEnvelope): Promise<void> {
		...
		if (envelope.packetType !== 'unsubscribe') {
			this.logError(`[WS_UNSUBSCRIBE] Invalid packet type for unsubscribe handler: ${envelope.packetType}`);
			this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid packet type');
			return;
		}
		...
		this.logDebug(`[WS_UNSUBSCRIBE] Unsubscribe request - clientId: ${clientId}, ` + `topic: ${topic}, channel: ${channelName}`);
	 ...
	 * @param envelope - Parsed packet envelope
	 */
	private async handlePublishPacket(ws: WebSocket, envelope: PacketEnvelope): Promise<void> {
		...
		if (envelope.packetType !== 'publish') {
			this.logError(`[WS_PUBLISH] Invalid packet type for publish handler: ${envelope.packetType}`);
			this.closeWebSocketWithError(ws, WsErrors.BadRequest, 'Invalid packet type');
			return;
		}
		...
		this.logDebug(`[WS_PUBLISH] Publish request - clientId: ${clientId}, topic: ${topic}, channel: ${channelName}`);
		...
		const hasWriteAccess = grant.data.topics.some((t: Grant['topics'][0]) => {
			...
			const scopeMatch = t.scope === Access.Write || t.scope === Access.ReadWrite;
		...
		if (!hasWriteAccess) {
			this.logError(`[WS_PUBLISH] Client lacks write access for topic "${topic}"`);
	 ...
	 */
	private async deliverMissedMessages(ws: WebSocket, grant: any, topic: string): Promise<void> {
		this.logDebug(`[DELIVER_MISSED] Fetching missed messages for topic: ${topic}`);

		try {
			// Get last seen sequence for this client
			const lastSeqSeen = await this.messageBuffer.getLastSeen(grant.project_id, grant.channel, topic, grant.userId);
			...
			this.logDebug(`[DELIVER_MISSED] Retrieved ${messages.length} missed messages`);

			// Send each missed message to the client
			let lastDeliveredSeq: string | null = null;
			for (const message of messages) {
				if (ws.readyState === WebSocket.READY_STATE_OPEN) {
					this.logDebug(`[DELIVER_MISSED] Sending missed message with seq: ${message.seq}`);
					ws.send(JSON.stringify(message));
					lastDeliveredSeq = message.seq;
				} else {
					this.logDebug(`[DELIVER_MISSED] WebSocket closed, stopping message delivery`);
					break;
				}
			}
			...
			if (lastDeliveredSeq) {
				await this.messageBuffer.updateLastSeenSingle(grant.project_id, grant.channel, topic, grant.userId, lastDeliveredSeq);
				this.logDebug(`[DELIVER_MISSED] Updated last-seen to seq=${lastDeliveredSeq} for clientId=${grant.userId}`);
			}

			this.logDebug(`[DELIVER_MISSED] Missed message delivery completed`);
		} catch (error) {
			this.logError(`[DELIVER_MISSED] Error delivering missed messages: ${error}`);
	...
	private getValidGrant(ws: WebSocket, operation: string): any {
		...
		if (!grant.success) {
			this.logError(`[WS_${operation.toUpperCase()}] Invalid grant schema: ${grant.error}`);
	 ...
	 */
	private closeWebSocketWithError(ws: WebSocket, code: number, reason: string): void {
		try {
			if (ws.readyState === WebSocket.READY_STATE_OPEN) {
				ws.close(code, reason);
				this.logDebug(`[CLOSE_WS] WebSocket closed with code ${code}: ${reason}`);
			}
		} catch (error) {
