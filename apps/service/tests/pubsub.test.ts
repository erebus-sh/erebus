...
const once = <T extends Event>(emitter: EventTarget, type: string) =>
	new Promise<T>((resolve) => emitter.addEventListener(type, (e) => resolve(e as T), { once: true }));
 ...
 */
async function createWebSocketConnection(locationHint = 'test-location', grantToken = token): Promise<WebSocketConnection> {
 ...
 */
async function connectToChannel(ws: WebSocket, grantToken = token): Promise<void> {
	const connectPacket = {
		packetType: 'connect',
		grantJWT: grantToken,
	};
 ...
 */
function setupMessageListener(ws: WebSocket, publishTimestamps?: Map<string, number>): MessageListener {
	const messages: any[] = [];
	...
	const messageHandler = (event: MessageEvent) => {
		...
		try {
			...
			const { clientMsgId, clientPublishTs } =
				typeof payload.payload === 'object' ? payload.payload : { clientMsgId: undefined, clientPublishTs: undefined };

			// Calculate publish time from correlation map or client timestamp
			const publishTime = clientMsgId && publishTimestamps ? publishTimestamps.get(clientMsgId) : (clientPublishTs ?? undefined);

			const serverSentAtWall = new Date(payload.sentAt).getTime();

			// Calculate latencies
			const publishToServerLatency = publishTime ? (payload.t_ingress ?? serverSentAtWall) - publishTime : undefined;
			const serverToClientLatency = receivedAt - serverSentAtWall;
			const totalRoundtripLatency = publishTime ? receivedAt - publishTime : undefined;
 ...
 */
async function closeConnection(ws: WebSocket, reason = 'test complete'): Promise<void> {
...
describe('WebSocket /v1/pubsub channel two clients tests', () => {
	...
	it(
		...
		async () => {
			...
			const publisherConnection = await createWebSocketConnection(sharedLocationHint, token);
			const subscriberConnection = await createWebSocketConnection(sharedLocationHint, token1);
			...
			const messageListener = setupMessageListener(subscriberConnection.ws, publishTimestamps);
			...
			for (let i = 1; i <= messageCount; i++) {
				...
				await publishMessage(publisherConnection.ws, testTopic, messageContent, `msg-${i}`, `publisher-client`);
			...
			for (let i = 0; i < messageCount; i++) {
				...
				const clientMsgId =
					msg.payload && typeof msg.payload === 'object' && 'clientMsgId' in msg.payload ? msg.payload.clientMsgId : 'no-id';
				...
				console.log(`  t_broadcast_begin: ${msg.t_broadcast_begin?.toFixed(3)}ms`);
				console.log(`  t_ws_write_end: ${msg.t_ws_write_end?.toFixed(3)}ms`);
				console.log(`  t_broadcast_end: ${msg.t_broadcast_end?.toFixed(3)}ms`);
				console.log(`  publishToServerLatency: ${msg.publishToServerLatency?.toFixed(2)}ms`);
				console.log(`  serverToClientLatency: ${msg.serverToClientLatency?.toFixed(2)}ms`);
				console.log(`  totalRoundtripLatency: ${msg.totalRoundtripLatency?.toFixed(2)}ms`);
				...
				if (msg.t_broadcast_begin && msg.t_ws_write_end) {
					expect(msg.t_ws_write_end).toBeGreaterThanOrEqual(msg.t_broadcast_begin);
				}
			}
			...
			const ingressToBroadcastStats = calculatePercentiles(ingressToBroadcastLatencies);
			const broadcastToWriteEndStats = calculatePercentiles(broadcastToWriteEndLatencies);
			...
			for (let i = 0; i < messageCount; i++) {
				...
				const matchingMessage = messageListener.messages.find((msg) => {
					...
					return payload && typeof payload === 'object' && 'clientMsgId' in payload && payload.clientMsgId === expectedClientMsgId;
				});
			...
			await closeConnection(subscriberConnection.ws, 'subscriber test complete');
		},
		testTimeout,
