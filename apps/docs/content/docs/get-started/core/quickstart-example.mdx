...
<Callout title="Install" type="info">
  In this example for the API endpoint I will be using Bun.Serve, you can use
  any other server framework you prefer.
</Callout>
...
If you’re running locally, also set the `wsBaseUrl`; otherwise it defaults to `wss://gateway.erebus.sh`.
...
Full code example: [examples/chat-ts/index.ts](https://github.com/erebus-sh/erebus/blob/master/examples/chat-ts/index.ts)
```
