...
Each key in the schema represents a _channel type_ (e.g. `news`, `chat`, `presence`), and defines the payload shape for that channel.
...
<Callout title="Good to know" type="note">
  The Schema Facade automatically combines the **schema key** and the **topic
  ID** into a full channel name. For example, using `("news", "sports")` becomes
  the topic string `news_sports` behind the scenes.
</Callout>
