...
Check the example in [Chat Typescript](https://github.com/erebus-sh/erebus/tree/master/examples/chat-ts)

### ErebusClient

- `ErebusClient.createClient({ client, authBaseUrl, wsBaseUrl })` → returns a Pub/Sub client when `client: ErebusClientState.PubSub`.
- `joinChannel(channel: string)` → sets the channel before connecting.
- `connect()` → establishes the WebSocket connection (uses grants from `authBaseUrl`).
- `subscribe(topic: string, handler)` → receive real-time messages.
- `onPresence(topic: string, handler)` → receive presence updates for a topic.
- `publishWithAck({ topic, messageBody, onAck, timeoutMs? })` → publish and handle ack.

### Types and helpers

- `ErebusClientState.PubSub` – select the Pub/Sub client.
- `Access.ReadWrite` – grant read/write permissions in sessions.
- `createGenericAdapter({ authorize, fireWebhook? })` – minimal Bun adapter for auth + webhooks.