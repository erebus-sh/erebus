...
<Callout title="Authentication Required" type="info">
  Erebus requires JWT tokens for authentication. The token must include your API
  key in the `erebus_api_key` field. See our [authentication
  guide](/docs/sdk/core/getting-started-api#authentication) for detailed setup.
</Callout>
