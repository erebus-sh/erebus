...
_[Screenshot placeholder: API keys dashboard showing key list with status indicators]_
...
_[Screenshot placeholder: API key card showing different states and action buttons]_
...
<Callout title="Key State Best Practices" type="info">
  Use the disabled state for temporary suspensions (debugging, maintenance) and
  the revoked state only for permanent deactivation when keys are compromised or
  definitively no longer needed.
</Callout>
...
_[Screenshot placeholder: Create API key dialog with environment selection]_
...
_[Screenshot placeholder: API key usage analytics showing connection metrics]_
...
- **[Authentication Guide](/docs/concepts/authentication)** - Authentication and security practices
